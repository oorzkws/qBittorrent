...
bool ApplicationInstanceManager::isFirstInstance()
{
    const bool isFirstInstance = !m_peer->isClient();

#ifdef Q_OS_WIN
    QSharedMemory *sharedMem = new QSharedMemory {appId() + QLatin1String {"-shared-memory-key"}, this};
    if (isFirstInstance) {
...
bool ApplicationInstanceManager::sendMessage(const QString &message, int timeout)
{
