...
try
{
    ...
    if (!isHidden)
    {
        QProgressDialog *progressDialog = createStartupProgressDialog();
        progressDialog->setMinimumDuration(1000);
        if (pref->startMinimized())
            progressDialog->setWindowState(Qt::WindowMinimized);
    }
    else
    {
#if (QT_VERSION < QT_VERSION_CHECK(6, 0, 0))
...
#ifndef DISABLE_GUI
QProgressDialog *Application::createStartupProgressDialog()
{
    ...
    progressDialog->setWindowFlag(Qt::WindowMinimizeButtonHint, true);
    progressDialog->setMinimumDuration(0);
    progressDialog->setAutoReset(false);
    progressDialog->setAutoClose(false);
    progressDialog->setLabelText(tr("Loading torrents..."));
    ...
    connect(BitTorrent::Session::instance(), &BitTorrent::Session::startupProgressUpdated, progressDialog, &QProgressDialog::setValue);
    connect(BitTorrent::Session::instance(), &BitTorrent::Session::restored, progressDialog, &QObject::deleteLater);

    connect(m_desktopIntegration, &DesktopIntegration::activationRequested, progressDialog, [progressDialog]()
    {
#ifdef Q_OS_MACOS
        ...
        if (progressDialog->isHidden())
        {
            ...
            progressDialog->setWindowState((progressDialog->windowState() & ~Qt::WindowMinimized) | Qt::WindowActive);
